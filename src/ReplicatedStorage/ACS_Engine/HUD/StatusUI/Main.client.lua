...
function getNearestBreach()
	...
	for I,Door in pairs (BreachFolder:GetChildren()) do
		if Door:FindFirstChild("Destroyable") ~= nil and Door.Destroyable:FindFirstChild("Charge") ~= nil then
...
function ResetGui()
	...
	GUI.Sistema.BandagesAberto.Visible = false
	GUI.Sistema.MedicineAberto.Visible = false
	GUI.Sistema.OtherAberto.Visible = false
end
...
GUI.InteractionMenu.Porta:WaitForChild('Abrir').MouseButton1Click:connect(function()
	...
	local Key
	DoorEvent:FireServer(nearestDoor,2,Key)
end)
		...
		if (player.Character:FindFirstChild("HumanoidRootPart")) and (player.Character.HumanoidRootPart.Position - mouse.Target.Position).magnitude <= 100 then
			if game.Players:FindFirstChild(mouse.Target.Parent.Name) or game.Players:FindFirstChild(mouse.Target.Parent.Parent.Name) then
				...
				if playera.Team == player.Team then
					...
					interactionmouse.Fundo.Username.TextStrokeTransparency = (player.Character.HumanoidRootPart.Position - mouse.Target.Position).magnitude / 100
					interactionmouse.Fundo.Username.TextTransparency = (player.Character.HumanoidRootPart.Position - mouse.Target.Position).magnitude / 100
				else
