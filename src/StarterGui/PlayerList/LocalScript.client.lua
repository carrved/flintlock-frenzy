...
local function SetupStateOverlay(B)
	...
	local function SetImage(t, c) SO.ImageTransparency = t SO.ImageColor3 = c end
    
    local function UpdateState()
		if IsPressed then SetImage(0.7, Color3.new(0, 0, 0))
        elseif IsHovered then SetImage(0.9, Color3.new(1, 1, 1))
        else SetImage(1, Color3.new(1, 1, 1)) end
    end
    
	local function MouseButton1Down()
		if CurrentButton ~= B then
			IsPressed = true UpdateState()
		end
	end
    local function MouseButton1Up() IsHovered = false IsPressed = false UpdateState() end

    local function MouseLeave()IsHovered = false IsPressed = false UpdateState() end
    ...
    B.MouseButton1Down:Connect(MouseButton1Down) B.MouseButton1Up:Connect(MouseButton1Up)
   	B.MouseEnter:Connect(MouseEnter) B.MouseLeave:Connect(MouseLeave)
end
